...
clear all; clc;
datadir = '/imaging/projects/cbu/ntad/meg_data/'; % input file directory 
maskdir='/imaging/ek01/atlases/AAL/names_long/'; % mask file directory
outdir='/imaging/ek01/ntad/mmn_dcm/dcm_input/r0_neg_N100/'; % output directory
...
subs = {'C1001','C1002','C1003','C1004','C1006','C1007',...
   ...
   'C1016','C1017','P1002','P1004','P1005','P1007','P1008','P1009',...
    ...
    'P1064','P1065','P1066','P1067','P1070','P3002','P3004','P3005'}; % baseline data
...
% Filename of ROI masks and nickname used in naming new files
...
reslice=0; % reslice mask 
newvoxel=[1 1 1]; % reslice voxel size
...
% Options
wholebrain=0; % extract from every vertex, (normal mesh 8196 vertices. if you want more you need to increase your mesh size)
usemask =0; % extract from mask
virtualelectrode=1;% extract from a virtual electrode ie coordinates
val = 1;  % inversion number set by source recon
virtual_el_coor=[-46 -20 8;
    -61 -32 8;
    -42 -22 7;
    46 20 8;
    59 -25 8;
    46 -14 8]; % coordinates for virtual electrode switch
...
for mask = 1:length(masknic)
    ...
    if usemask
        ...
        if homo % for homologous (symmetric on x axis) ROIs
            x = XYZ_all(:,1).*-1;
            XYZ_all(:,1) = x; clear x
        end
...
for sub = 1:length(subs)
    ...
    for mask=1:length(masknic) % Dipole flip by local maxima at N100
        for tr=1:size(Ds,3)
