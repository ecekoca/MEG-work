%% Preprocessing script for scenerep
...
epochWin = [-100 800]; % epoch window in ms
con_values = [22 33]; % trigger values that correspond to your conditions of interest
offset = [35 35]; % stimulus delivery delay for each condition (NI card & Presentation; Audio: 64 ms; Vis: 35 ms)
...
if dorename
    ...
    for sub=1:length(subjects)
        ...
        S.outfile = [ses_dir '/Maffffd' basefile '_new_lbl.mat'];
        D= spm_eeg_copy(S);
    end
    
end

basefile=[basefile '_new_lbl'];
%% Read triggers & Epoch
%%=========================================================================
...
%     parfor sub=1:length(subjects)
    for sub=1:length(subjects)
        if exist(fullfile(ana_dir,subjects{sub},ses_nam,'/',task),'dir')
            ...
            for o = 1:length(ons)
                ...
                trl(end+1,1) = ons(o) + round((epochWin(1) + pads(1) + offset(con))*(D.fsample/1000));
                trl(end,2)   = ons(o) + round((epochWin(2) + pads(2) + offset(con))*(D.fsample/1000));
                trl(end,3)   = round((epochWin(1) + pads(1))*(D.fsample/1000));
            end
    ...
    for sub = 1:length(subjects)
        if exist(fullfile(ana_dir,subjects{sub},ses_nam,task))
            ...
            trlfile = fullfile(ses_dir,sprintf('%strl_new_lbl.mat',''));
    ...
    %for sub=1:length(subjects)
        ...
        ses_dir = [ana_dir subjects{sub} '/' ses_nam '/' task '/']
        trl = load(fullfile(ses_dir,sprintf('%strl_new_lbl.mat','')));
...
%%=========================================================================
 if dorename2
    ...
    for sub=1:length(subjects)
        ...
        S.outfile = [ses_dir '/' subjects{sub} '_' ses_nam '_' task '_new_lbl.mat'];
        D = spm_eeg_copy(S);
    end
