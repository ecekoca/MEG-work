function Do = spm_eeg_grandmean_ek(S)
...
% if ischar(D)
    ...
    try
        for i = 1:size(D, 1)
            %F{i} = spm_eeg_load(deblank(D(i, :))); %EK
            F{i} = spm_eeg_load(deblank(D{i,1}));
        end
        D = F;
    catch
...
if strncmp(D{1}.transformtype, 'TF',2)
    ...
    for i = 1:Ntypes
        ...
        f = zeros(D{1}.nchannels, 1); %EK
        for j = 1:D{1}.nchannels
            
            for k = 1:Nfiles
                if ~any(badchans{k}==j) 
                    ind = trialinds{k}(i);
                    if ~isempty(ind) && le(j,size(D{k},1)) %EK
                        d(j, :) = d(j, :) + nrepl(k, i)*D{k}(j, :, ind); 
                        %w(j, i) = w(j, i) + nrepl(k, i);
                        f(j,1)=f(j,1)+1;%EK
                    end
