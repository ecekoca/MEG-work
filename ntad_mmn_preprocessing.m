...
subjects = {'C1001','C1002','C1003','C1004','C1006','C1007',...
'C1008','C1010','C1011','C1012','C1013','C1014','C1015','C1016','C1017',...
'C2001','C2002','C2008','P1001','P1002','P1004','P1005','P1007','P1008','P1009',...
...
'P1064','P1065','P1066','P1067','P1070','P3002','P3004','P3005','P3006'};
...
dorename1= 1; 
doepoch = 1; % Epoch
doartefact2 = 1; % Second artefact rejection
doaverage = 1; % Average trials and low pass
docontrast = 1; % Compute contrasts
dofilter2 = 1; % Filter to analysis-specific frequency band
removeeeg = 0;
dorename2 = 1; % Rename files to something sensible

% Parameters
basefile = 'transdef'; %Maxfilter output to preprocess
...
offset = ones(1,length(con_values)).*64; % stimulus delivery delay for each condition (NI card & Presentation; Audio: 64 ms; Vis: 35 ms)
...
if doconvert
    chanfile = fullfile('chan_select_TGB_MEG_EEG_STI101.mat'); %EK
    %chanfile = fullfile('chan_neuromag_triux_374.mat'); %EK
    load(chanfile); % chan=load(chanfile);
    %ref_old_names = {'EOG061','EOG062','ECG063'};
    ref_old_names = {'EOG001','EOG002','ECG003'};
...
if doartefact % uses Oxford's OSL, works much better than SPM's.
    for sub=1:length(subjects)
        ...
        try
            D2=osl_detect_artefacts(D,'modalities',{'EEG','MEGMAG','MEGPLANAR'});
        catch
            D2=osl_detect_artefacts(D,'modalities',{'MEGMAG','MEGPLANAR'});
        end
        D=D2;
...
if doica
    ...
    arttopos = load('/imaging/projects/cbu/ntad/scripts/MEGEEG70ArtifactTemplateTopographies');
    sparefs = {};
    ...
    parfor sub=1:length(subjects)
        ...
        if ~isempty(find(ismember(D.chanlabels,'EOG061')))
            ...
            if ~isempty(find(ismember(D.chanlabels,'VEOG'))) %EK
                S.refs.tem{1}=D(find(ismember(D.chanlabels,'VEOG')),:);
                S.refs.tem{2}=D(find(ismember(D.chanlabels,'HEOG')),:);
                S.refs.tem{3}=D(find(ismember(D.chanlabels,'ECG')),:);
            else
                S.refs.tem{1}=D(find(ismember(D.chanlabels,'EOG062')),:);
                S.refs.tem{2}=D(find(ismember(D.chanlabels,'EOG061')),:);
                S.refs.tem{3}=D(find(ismember(D.chanlabels,'ECG063')),:);%
...
%%=========================================================================
 if dorename1
    ...
    for sub=1:length(subjects)
        ses_dir = fullfile(ana_dir,subjects{sub},ses_nam,task)
        S = [];
        S.D = [ses_dir '/Maffffd' basefile '.mat'];
        S.outfile = [ses_dir '/Maffffd' basefile '_all_trl.mat'];
        D = spm_eeg_copy(S);
    end
...
if doepoch
   ...
   parfor sub=1:length(subjects)
        ...
        if exist(fullfile(ana_dir,subjects{sub},ses_nam,'/',task),'dir')
            ...
            D = spm_eeg_load([ses_dir 'd' basefile '.mat']);
    ...
    for sub = 1:length(subjects) % Don't put it in parfor to make sure file is saved in the correct directory
        if exist(fullfile(ana_dir,subjects{sub},ses_nam,task))
            ses_nam = session;
            ses_dir = fullfile(ana_dir,subjects{sub},ses_nam,task);
            trl = all_trl{sub}; conditionlabels = all_con{sub};
            trlfile = fullfile(ses_dir,sprintf('%s_trl_all_trl.mat','transdef')); 
    ...
    parfor sub=1:length(subjects)
        ...
        ses_dir = [ana_dir subjects{sub} '/' ses_nam '/' task '/']; cd(ses_dir)
        trl = load(fullfile(ses_dir,sprintf('%s_trl_all_trl.mat',basefile)));
        D = spm_eeg_load([ses_dir '/Maffffd' basefile '_all_trl.mat']); %EK
        S = []; S.D = D;
        S.trl = trl.trl;
...
%%=========================================================================
%% Artefact rejection and removal (EK)
%%=========================================================================
...
if doartefact2 % EK
    ...
    for sub=1:length(subjects)
        ...
        ses_dir = [ana_dir subjects{sub} '/' ses_nam '/' task '/']; cd(ses_dir)
        D = spm_eeg_load([ses_dir '/eMaffffd' basefile '_all_trl.mat']); %EK
        
        if ~isequal(max(ismember(D.chantype,'MEGMAG')),1)
            load([ses_dir '/eMaffffd' basefile '_all_trl.mat'])
            ...
            end; save([ses_dir '/eMaffffd' basefile '_all_trl.mat'],'D','-v7.3')
            clear D; D = spm_eeg_load([ses_dir '/eMaffffd' basefile '_all_trl.mat']); %EK
        end
        D=D.badchannels(1:D.nchannels,0);
        D2=osl_detect_artefacts(D,'modalities',{'MEGMAG','MEGPLANAR'});D=D2;
        save([ses_dir '/aeMaffffd' basefile '_all_trl.mat'],'D','-v7.3')
        D = spm_eeg_load([ses_dir '/aeMaffffd' basefile '_all_trl.mat']); %EK
        S = []; S.D = D;
...
if doaverage %EK
    ...
    parfor sub=1:length(subjects)
        ...
        ses_dir = [ana_dir subjects{sub} '/' ses_nam '/' task '/']
        D = spm_eeg_load([ses_dir '/eMaffffd' basefile '_all_trl.mat']);
        S   = []; S.D = D;
        S.robust.savew = 0;
    ...
    parfor sub=1:length(subjects)
        ...
        ses_dir = fullfile(ana_dir,subjects{sub},ses_nam,task)
        S = [];
        S.D = [ses_dir '/meMaffffd' basefile '_all_trl.mat'];
        S.type = 'butterworth';
        S.order = 5;
        S.band = 'low';
        S.freq = low;
...
if docontrast
    ...
    parfor sub=1:length(subjects)
        ...
        ses_dir = [ana_dir subjects{sub} '/' ses_nam '/' task '/']
        D = spm_eeg_load([ses_dir '/fmeMaffffd' basefile '_all_trl.mat']);
        
        S = [];
        S.D = D;
        S.label = {'DEV','REP1','REP2','REP3','REP4','REP5',...
            'REP6','REP7','REP8','REP9','REP10','DEV-STD'};
        S.c = zeros(12,11);
        i=find(strcmp(D.conditions,'DEV')); %EK
        S.c(1,i) = 1;
        j=find(strcmp(D.conditions,'REP1'));
        S.c(2,j) = 1;
        k=find(strcmp(D.conditions,'REP2'));
        S.c(3,k) = 1;
        k=find(strcmp(D.conditions,'REP3'));
        S.c(4,k) = 1;
        k=find(strcmp(D.conditions,'REP4'));
        S.c(5,k) = 1;
        k=find(strcmp(D.conditions,'REP5'));
        S.c(6,k) = 1;
        k=find(strcmp(D.conditions,'REP6'));
        S.c(7,k) = 1
        k=find(strcmp(D.conditions,'REP7'));
        S.c(8,k) = 1;
        k=find(strcmp(D.conditions,'REP8'));
        S.c(9,k) = 1;
        k=find(strcmp(D.conditions,'REP9'));
        S.c(10,k) = 1;
        k=find(strcmp(D.conditions,'REP10'));
        S.c(11,k) = 1;
        k=find(strcmp(D.conditions,'DEV'));k2=find(strcmp(D.conditions,'REP5'));
        S.c(12,k) = 1;S.c(12,k2) = -1;
...
if dofilter2
    ...
    parfor sub=1:length(subjects)
        ses_dir = fullfile(ana_dir,subjects{sub},ses_nam,task)
        S = [];
        S.D = [ses_dir '/wfmeMaffffd' basefile '_all_trl.mat'];
        S.type = 'butterworth';
        S.order = 5;
        S.band = 'low';
        S.freq = low;
...
if removeeeg % Not necessary, only use to fix grand averaging bug
    for sub=1:length(subjects)
        ses_dir = fullfile(ana_dir,subjects{sub},ses_nam,task)
        S = [];
        S.D = [ses_dir '/wfmeMaffffd' basefile '_all_trl.mat'];
        S.channels={'MEGPLANAR','MEGMAG'};
        D=spm_eeg_crop(S);
    end
end
...
if dorename2
    ...
    for sub=1:length(subjects)
        ses_dir = fullfile(ana_dir,subjects{sub},ses_nam,task)
        S = [];
        S.D = [ses_dir '/fwfmeMaffffd' basefile '_all_trl.mat'];
        S.outfile = [ses_dir '/' subjects{sub} '_' ses_nam '_' task '_all_trl.mat'];
        D = spm_eeg_copy(S);
    end
